...
void displayMap(string map[], size_t size)
{
  ...
  cout << "\033[1m" << setw(7) << "Points:"
       << "\033[0m" << setw(2) << Player.points << "\t\033[1mLives:\033[0m" << setw(2) << Player.lives << "\t\033[1mHealth: \033[0m" << setw(2) << Player.health << "\t\033[1mSecrets: \033[0m" << setw(2) << Player.currentSecretsCompleted << endl;
...
void endScenario(int *x, int *y, bool &scenarioCondition)
{
  ...
  // render the map3 again
  cout << endl; // !so the map3 is rendered one line below
  displayMap(map3, 34);
    ...
    else if (enteredIntelRoom)
    {
      ...
      */
      ...
      while (tries > 0 && !isCorrect)
      {
        ...
        {
          cout << "You have successfully decoded the secret.";
      ...
      else
      {
        ...
        */
        ...
        if (Player.currentSecretsCompleted == 3)
        {
          ...
          if (Player.health <= 0)
          {
            restartLevel(x, y, "Oops! You have the lost with the final boss.\n");
            endScenario(&x, &y, enteredIntelRoom);
          }
          else
          {
            animateText("You have defeated the final boss. Deer kha alaka!!!", false);
            system("pause");
            exit(0);
          }
        }
