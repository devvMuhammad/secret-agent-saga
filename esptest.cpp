...
void loadingScreen()
{
  cout << "\tLoading the next level ..." << endl;
  cout << "\t";
  for (int i = 0; i < 60; i++)
  {
    printf("%c", 177);
...
void displayMap(string map[], size_t size)
{
  cout << "\033[1m" << setw(7) << left << "Stage:"
       << "\033[0m" << setw(2) << left << Player.currentStage << "\t\033[1mObjectives:\033[0m "
       << "1 Kill and 1 Secret Code" << endl;
  cout << "\033[1m" << setw(7) << left << "Points:"
       << "\033[0m" << setw(2) << left << Player.points << "\t\033[1mLives:\033[0m" << setw(2) << Player.lives << "    \033[1mHealth: \033[0m" << setw(2) << left << Player.health << endl;
  cout << "\033[1m" << setw(7) << left << "Ammo:"
       << "\033[0m" << setw(2) << left << Player.ammo << "\t\033[1mSmokes:\033[0m" << setw(2) << Player.smokes << endl;
...
void animation2(string direction, int *x, int *y, int *ax, int *ay, int *bx, int *by, bool &collisionOcurred, bool &caughtByCamera, bool &enteredIntelRoom)
{
  for (int i = 0; i < 3; ++i)
  {
    ...
    if (!caughtByCamera || !enteredIntelRoom)
      moveCharacter(x, y);
    ...
    else if (enteredIntelRoom)
    {
      ...
      */
      ...
      while (tries > 0 && !isCorrect)
      {
        ...
        if (userInput == question.answer)
        {
          cout << "You have successfully decoded the secret. You now move on to level 3";
      ...
      if (!isCorrect)
      {
        restartLevel2(x, y, "Oops! You are out of tries. You will have to start again. Better luck next time.\n");
